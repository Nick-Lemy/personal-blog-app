...
function Home() {
  ...
  const fetchPosts = async () => {
    ...
    console.log(token)
    const posts = await axios.get(`${BACKEND_BASE_URL}/post/all`, {
      headers: {
      ...
      <div className="flex-1 p-5 md:p-10 border-2 m-2 border-gray-600 bg-gray-900 rounded-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {allPosts && (allPosts.length > 0 ?
            allPosts.map(post => (
              <Card date={post.created_at} key={post._id} title={post.title} subtitle={post.subtitle} color={{ bg: 'bg-blue-950', txt: 'text-white' }} id={post._id} />
            )) : (
